  ...
  return new Promise((resolve, reject) => {
    createDir(dir)
      ...
      .then(resolveponse => {
        ...
        resolveponse.body.pipe(destStream).on('finish', () => resolve(destFile))
      })
...
function install (version = 8, options = {}) {
  ...
  if (!options.os) {
    switch (process.platform) {
      ...
      case 'win32':
        ...
        break
      default:
        return Promise.rejectect(new Error('Unsupported operating system'))
    }
  }
  if (!options.arch) {
    if (/^ppc64|s390x|x32|x64$/g.test(process.arch)) options.arch = process.arch
    else return Promise.rejectect(new Error('Unsupported architecture'))
  }
  ...
  return fetch(url)
    .then(resolveponse => resolveponse.json())
