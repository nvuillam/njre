    ...
    yauzl.open(file, { lazyEntries: true }, (err, zipFile) => {
      ...
      zipFile.on("entry", (entry) => {
        ...
        if (/\/$/.test(entry.fileName)) {
          // Directory entry: create it then move on
          fs.mkdir(entryPath, { recursive: true }, (err) => {
            if (err && err.code !== "EEXIST") return reject(err);
            zipFile.readEntry();
          });
        } else {
          // File entry: ensure the parent dir exists (zip entries are not
          // guaranteed to list directories first), then stream to disk and
          // wait for the WRITE to finish before reading the next entry. This
          // bounds open file handles to one and guarantees each file is fully
          // flushed; without it, a stalled/failed write on Windows means the
          // read stream never ends, readEntry() is never called again, and the
          // zip "close" event never fires (the install hangs).
