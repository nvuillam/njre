...
function install(version = 8, options = {}) {
  ...
  if (options.vendor === "adoptopenjdk") endpoint = "api.adoptopenjdk.net";
    ...
    return Promise.reject(
      new Error(`[njre] Unsupported vendor ${options.vendor}. Use adoptopenjdk (default) or eclipse")`));
  ...
  if (!options.os) {
    switch (process.platform) {
      ...
      case "win32":
        ...
        break;
      default:
        return Promise.reject(new Error(`[njre] Unsupported operating system ${process.platform}`));
    }
  }
  if (!options.arch) {
    ...
    else return Promise.reject(new Error(`[njre] Unsupported architecture ${process.arch}`));
  }
