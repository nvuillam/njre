...
```bash
yarn install          # ALWAYS use yarn, not npm: the repo relies on yarn.lock and the resolutions field of package.json
yarn test             # mocha suite (downloads real JRE/JDK binaries, needs network, takes a few minutes)
yarn coverage         # same suite under nyc
