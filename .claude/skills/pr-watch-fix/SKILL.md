...
If the same check has been pending more than **30 minutes** without a state change, the Monitor must emit a `[stalled]` event and you should **ask the user** whether to keep waiting. (The cross-platform test matrix - especially the Windows runner downloading a JRE - can legitimately take 10-15 min; MegaLinter a few more. Don't panic early.) Do not poll faster than 5 minutes.

### 5. Fix the failures (delegate to `pr-fix`)

Spawn the `pr-fix` agent (Opus) via the Agent tool. Pass it the branch, PR number, current HEAD SHA, and the failure list from `pr-watch` (job names, error types, key log lines). `pr-fix` owns the diagnosis, the fix, local validation (`yarn test` / `yarn lint` / `yarn docs`), and the commit + push (including the MegaLinter `--force-with-lease` reconcile and all the git-safety rules).
...
```
Cycle 2: Test (windows-latest) failed (assertion in tests/test.js:42), pushed e0a44f1. Waiting 5m.
```
...
- Never edit generated files (`DOCS.md` is regenerated from JSDoc by `yarn docs`), never bypass Husky hooks (`--no-verify`), use **Yarn** not npm (CI and `yarn.lock` are the source of truth). `pr-fix` enforces these; if it reports a violation it could not avoid, stop and ask.
